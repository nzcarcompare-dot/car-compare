...
function updateChartTheme() {
  ...
  const tooltipTitle = dark ? '#e6edf3' : '#0f172a';
  const tooltipBody = dark ? '#8b949e' : '#64748b';
...
function renderList(car, query) {
  ...
  const html = filtered.slice(0, 14).map(v => {
    const chipClass = fuelChipClass[v.fuelType] || '';
    const economy = v.l100 ? v.l100 + ' L/100km' : v.kwh ? v.kwh + ' kWh/100km' : '';
    return `<div class="dropdown-item" data-car="${car}" data-plate="${v.plate}">
      <div class="di-name">${v.name}<span class="fuel-chip ${chipClass}">${v.fuelType.toUpperCase()}</span></div>
      <div class="di-meta">${[v.bodyType, economy, '$' + v.price.toLocaleString()].filter(Boolean).join(' · ')}</div>
    </div>`;
  }).join('') || '<div class="dropdown-item"><div class="di-meta">No matches — try the Manual tab</div></div>';
...
['a', 'b'].forEach(car => {
  el(car + '-search').addEventListener('input', () => {
    renderList(car, el(car + '-search').value);
    openList(car);
  });
...
async function lookupPlate(car) {
  ...
  try {
    ...
    Object.assign(state[car], {
      ...
      year: v.year || '', fuelType: v.fuelType || 'petrol', co2: v.co2 || 0,
      stars: v.stars || null, safety: v.safety || null, seats: v.seats || null,
      bodyType: v.bodyType || '', trans: v.trans || v.transmission || '',
      cc: v.cc || null, notes: v.notes || ''
    });
    ...
    setStatus(car, '✓ Found' + (v._demo ? ' (demo)' : ''), 's-ok');
  } catch (e) {
...
el('a-plate-input').addEventListener('keydown', e => { if (e.key === 'Enter') lookupPlate('a'); e.target.value = e.target.value.toUpperCase(); });
el('b-plate-input').addEventListener('keydown', e => { if (e.key === 'Enter') lookupPlate('b'); e.target.value = e.target.value.toUpperCase(); });
el('a-plate-input').addEventListener('input', e => { e.target.value = e.target.value.toUpperCase(); });
el('b-plate-input').addEventListener('input', e => { e.target.value = e.target.value.toUpperCase(); });
...
function showBanner(car, v) {
  ...
  el(car + '-vmeta').textContent = metaParts.join(' · ');
...
function hideBanner(car) { el(car + '-banner').classList.remove('show'); }
...
function fuelDesc(car) {
  return { petrol: 'Petrol', diesel: 'Diesel + RUC', ev: 'Electricity + RUC', hybrid: 'Hybrid (petrol)', phev: 'PHEV blend' }[el(car + '-fuel').value] || '';
}

/* ── Animated counter ──────────────────────────────────────────────────────── */
function animateCounter(elem, target, prefix, suffix) {
  const duration = 600;
  const start = performance.now();
  function step(now) {
    const progress = Math.min((now - start) / duration, 1);
    const ease = 1 - Math.pow(1 - progress, 3);
    elem.textContent = (prefix || '$') + Math.round(ease * target).toLocaleString() + (suffix || '');
...
function treeFact(co2A, co2B, km) {
  ...
  if (trees === 0) {
    return 'These vehicles have very similar emissions — less than one tree\'s worth of CO₂ difference per year.';
  }
  if (Math.min(co2A, co2B) === 0) {
    return `<span class="tree-num">${trees} trees/yr</span>The zero-emission option saves ${Math.round(diffKg).toLocaleString()} kg of CO₂ every year versus the ${hiName}.`;
  }
...
function compare() {
  ...
  for (let y = 0; y <= yrs; y++) {
    ...
    if (!breakEven && y > 0) {
      ...
      if (prev * curr < 0) {
        breakEven = (y - 1 + Math.abs(prev) / Math.abs(rA - rB)).toFixed(1);
      }
    }
  }
  ...
  if (breakEven) {
    ...
    const cheaper = (pA + rA * yrs) <= (pB + rB * yrs) ? nA : nB;
    badge.textContent = cheaper + ' cheaper over ' + yrs + ' yrs';
  ...
  const tickColor  = dark ? '#8b949e' : '#64748b';
  const tooltipBg  = dark ? '#1c2128' : '#ffffff';
  const tooltipBdr = dark ? '#30363d' : '#e2e8f0';
  const tooltipTtl = dark ? '#e6edf3' : '#0f172a';
  const tooltipBdy = dark ? '#8b949e' : '#64748b';
  ...
  chartInstance = new Chart(el('chart'), {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: tickColor, font: { size: 11 }, maxRotation: 0, autoSkip: true, maxTicksLimit: 12 }, grid: { color: gridColor }, border: { display: false } },
        y: { ticks: { callback: v => '$' + (v / 1000).toFixed(0) + 'k', color: tickColor, font: { size: 11 } }, grid: { color: gridColor }, border: { display: false } }
      }
    }
  });
  ...
  const cAm = Math.round(pA + rA * mid), cBm = Math.round(pB + rB * mid), aWinsMid = cAm <= cBm;
  ...
  el('cost-table').innerHTML =
    ...
    `<tr class="total-row"><td><strong>Total over ${yrs} yrs</strong></td><td class="${aWinsMid ? 'win-a' : ''}">$${Math.round(pA + rA * yrs).toLocaleString()}</td><td class="${!aWinsMid ? 'win-b' : ''}">$${Math.round(pB + rB * yrs).toLocaleString()}</td></tr>`;

  // CO2 + trees
  const co2A = state.a.co2 || 0, co2B = state.b.co2 || 0;
  const maxCO2 = Math.max(co2A, co2B, 1);
  const annCO2A = Math.round(co2A * km / 1000), annCO2B = Math.round(co2B * km / 1000);
  ...
  el('co2-bars').innerHTML =
    `<div class="co2-row">
      ...
      <div class="co2-val">${co2A === 0 ? 'Zero 🌱' : annCO2A.toLocaleString() + ' kg/yr'}</div>
    </div>
    <div class="co2-row">
      ...
      <div class="co2-val">${co2B === 0 ? 'Zero 🌱' : annCO2B.toLocaleString() + ' kg/yr'}</div>
    </div>`;

  // Full comparison table
  const r2 = (label, a, b) => `<tr><td>${label}</td><td>${a}</td><td>${b}</td></tr>`;
  const cat = label => `<tr class="cat-hd"><td colspan="3">${label}</td></tr>`;
  const starStr = (n, max) => n ? Array.from({ length: max }, (_, i) => i < n ? '★' : '☆').join('') + ' ' + n + '/' + max : '—';
  ...
  el('detail-table').innerHTML =
    ...
    r2('Annual CO₂',      co2A === 0 ? 'Zero' : annCO2A.toLocaleString() + ' kg', co2B === 0 ? 'Zero' : annCO2B.toLocaleString() + ' kg') +
    r2('Economy rating',  starStr(state.a.stars, 6),  starStr(state.b.stars, 6)) +
    cat('Safety') +
    r2('Safety rating', starStr(state.a.safety, 5), starStr(state.b.safety, 5)) +
  ...
  void resultsEl.offsetWidth; // force reflow to restart animation
...
async function init() {
  ...
  // Restore from URL if applicable
  restoreFromURL();
}
