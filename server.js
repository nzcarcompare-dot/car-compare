...
.logo-mark {
  ...
  background:linear-gradient(135deg,#0ecfb0,#3b82f6);
  display:flex;align-items:center;justify-content:center;
  font-size:17px;
  box-shadow:0 0 16px rgba(14,207,176,.3);
}
...
.nz-badge {
  ...
  border:1px solid rgba(14,207,176,.2);
}
...
.hero::before {
  ...
  background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(14,207,176,.07) 0%,transparent 70%);
  pointer-events:none;
}
...
input[type=range] {
  ...
  background:linear-gradient(to right,var(--teal) 0%,var(--teal) 30%,var(--bg-4) 30%,var(--bg-4) 100%);
}
...
.car-stripe {
  height:3px;border-radius:3px 3px 0 0;margin-bottom:14px;
  background:linear-gradient(90deg,var(--a-accent),rgba(59,130,246,.3));
}
...
.mode-btn {
  ...
  border-radius:6px;font-size:11px;font-weight:600;font-family:'DM Sans',sans-serif;
...
.plate-input {
  ...
  color:var(--tx-1);font-family:'DM Sans',sans-serif;outline:none;
...
.search-input {
  ...
  font-size:13px;color:var(--tx-1);font-family:'DM Sans',sans-serif;outline:none;
  transition:border-color .15s,box-shadow .15s;
}
...
.vbanner {
  ...
  border-radius:10px;padding:12px 14px;margin-bottom:12px;display:none;
...
.field label {
  ...
  text-transform:uppercase;color:var(--tx-2);margin-bottom:4px;
}
...
.section-title {
  ...
  text-transform:uppercase;color:var(--tx-2);margin-bottom:14px;
}
...
.compare-btn {
  ...
  padding:12px 28px;font-size:14px;font-weight:700;font-family:'DM Sans',sans-serif;
  cursor:pointer;transition:all .2s;
  box-shadow:0 0 20px rgba(14,207,176,.25);
  letter-spacing:-.01em;
}
...
.swap-btn {
  ...
  padding:11px 16px;font-size:13px;font-weight:600;font-family:'DM Sans',sans-serif;
  color:var(--tx-2);cursor:pointer;transition:all .15s;
...
.share-btn {
  ...
  color:var(--tx-2);background:var(--bg-3);border:1px solid var(--border-2);
...
.share-toast {
  ...
  background:var(--teal);color:#000;padding:10px 22px;border-radius:20px;
...
.smetric {
  padding:18px 18px;border-right:1px solid var(--border);
  position:relative;overflow:hidden;
}
...
.sm-value {
  ...
  color:var(--tx-1);letter-spacing:-.02em;margin-bottom:4px;
  transition:all .4s;
}
...
.tab-btn {
  flex:1;min-width:100px;padding:14px 16px;border:none;background:transparent;
  font-size:12px;font-weight:700;letter-spacing:.02em;font-family:'DM Sans',sans-serif;
...
.co2-fill {
  ...
  transition:width .8s cubic-bezier(.34,1.56,.64,1);min-width:4px;
}
...
.comp-table th {
  ...
  letter-spacing:.07em;color:var(--tx-2);
...
.comp-table .cat-hd td {
  background:var(--bg-3);font-size:10px;font-weight:700;
  text-transform:uppercase;letter-spacing:.07em;color:var(--tx-2);padding:6px 12px;
}
...
async function init(){
  ...
  ['pp','dp','ep','yrs'].forEach(k=>{if(p.get(k)) document.getElementById(k).value=p.get(k);});
  ['a','b'].forEach(c=>{
    ...
    if(p.get(c+'name')){
      state[c].name=p.get(c+'name'); state[c].co2=parseFloat(p.get(c+'co2'))||0;
      state[c].make=p.get(c+'make')||''; state[c].model=p.get(c+'model')||'';
...
// ── km ────────────────────────────────────────────────────────────────────────
function updateKm(){
  const v=parseInt(document.getElementById('km').value);
...
function selectVehicle(car,plate){
  ...
  document.getElementById(car+'-search').value=v.name; closeList(car);
  document.getElementById(car+'-price').value=v.price;
  document.getElementById(car+'-fuel').value=v.fuelType; uf(car);
  ...
  showBanner(car,v); setStatus(car,'');
}
...
async function lookupPlate(car){
  ...
  try{
    ...
    document.getElementById(car+'-price').value=v.price||'';
    document.getElementById(car+'-fuel').value=v.fuelType||'petrol'; uf(car);
    if(v.l100) document.getElementById(car+'-l100').value=v.l100;
    if(v.kwh)  document.getElementById(car+'-kwh').value=v.kwh;
    showBanner(car,{...v,...state[car]}); setStatus(car,'✓ Found'+(v._demo?' (demo)':''),'s-ok');
  }catch(e){ setStatus(car,'✗ '+e.message,'s-err'); }
...
function showBanner(car,v){
  ...
  document.getElementById(car+'-trademe').href=
    make&&model?'https://www.trademe.co.nz/a/motors/cars/'+make+'/'+model
...
function swapCars(){
  ...
  if(document.getElementById('a-banner').classList.contains('show')) showBanner('a',state.a);
  if(document.getElementById('b-banner').classList.contains('show')) showBanner('b',state.b);
}
...
function runCost(car){
  ...
  if(f==='phev'){const p=g(car+'-ppct')/100;return p*(g(car+'-pkwh')/100)*km*ep+(1-p)*(g(car+'-pl100')/100)*km*pp;}
...
function compare(){
  ...
  document.getElementById('detail-table').innerHTML=
    ...
    r2('Fuel consumption',g('a-l100')>0?g('a-l100').toFixed(1)+' L/100km':'-',g('b-l100')>0?g('b-l100').toFixed(1)+' L/100km':'-')+
    r2('Electric consumption',g('a-kwh')>0?g('a-kwh')+' kWh/100km':'-',g('b-kwh')>0?g('b-kwh')+' kWh/100km':'-')+
...
function shareResults(){
  ...
  ['a','b'].forEach(c=>{
    p.set(c+'name',state[c].name); p.set(c+'make',state[c].make||''); p.set(c+'model',state[c].model||'');
